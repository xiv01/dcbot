...
module.exports = {
	...
	async execute(interaction) {
        ...
        try {
            ...
            logEx(`${interactionUser.user.username}#${interactionUser.user.discriminator} banned @${member.user.username}#${member.user.discriminator}`);
            ...
            logEx(`${interactionUser.user.username}#${interactionUser.user.discriminator} tried to ban @${member.user.username}#${member.user.discriminator}`);
            await interaction.reply({ embeds: [banembed] });
            setTimeout(() => interaction.deleteReply().catch(() => { console.error("[error] unable to delete message (already deleted?)") }), 8000);
        }
	},
